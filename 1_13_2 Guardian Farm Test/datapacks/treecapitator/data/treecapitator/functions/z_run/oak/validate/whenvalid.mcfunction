...
execute as @p[scores={TC_BO=1,TC_UAXE=1}] run function treecapitator:z_damage/setdurability

#Detect logs
function treecapitator:z_run/oak/detect/firstlog

#tellraw @a [{"text":"Damage before calculation: "},{"score":{"name":"Damage","objective":"TC_Value"}}]

#Edit damage if unbreaking enchantment
execute as @p[scores={TC_BO=1,TC_UAXE=1}] run function treecapitator:z_damage/calculate
...
#Cut tree
execute if score Durability TC_Value >= Damage TC_Value as @p[scores={TC_BO=1,TC_UAXE=1}] run function treecapitator:z_break/main